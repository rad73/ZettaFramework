...
class Modules_Zfdebuginit_Plugin_Widget extends Zend_Controller_Plugin_Abstract
{
    ...
    public function dispatchLoopShutdown() {
        if ($this->_isEnable) {
            ...
            if ($userAgent->getHttpAccept() == 'application/json') {
                $view = Zend_Controller_Action_HelperBroker::getStaticHelper('viewRenderer')->view;
                $view->zfdebug = Zetta_Log_Writers_Memory::getEvents();
    ...
    protected function _registerPanel()
    {
        ...
        if ($this->_isEnable || isset($_REQUEST[$_config->getvar])) {
            ...
            } elseif (Zend_Registry::isRegistered('db') && $db = Zend_Registry::get('db') && isset($dbKey)) {
