...
class Modules_Publications_AdminController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        if ($rubric_id = $this->getParam('rubric_id')) {
            ...
            if (class_exists('Publications\\Model\\' . ucfirst($this->_rubric->table_name))) {
                $className = 'Publications\\Model\\' . ucfirst($this->_rubric->table_name);
    ...
    public function viewAction()
    {
        ...
        try {	// пробуем подключить пользовательский шаблон
            $this->renderScript('admin_' . $this->_rubric->table_name . '/view.ajax.phtml');
        } catch (Exception $e) {
        }
    }
    ...
    public function addpublicationAction()
    {
        ...
        if (!sizeof($_POST) || !$form->isValid($_POST)) {
            ...
            if ($publication_id) {
                ...
                $publication_id = $this->_modelPublications->insert($post);
            }

            \Zetta\Hook::call('publications__admin_addpublication', [
                'publication_id' => $publication_id,
                'rubric_id'      => $this->_rubric->rubric_id,
                'data'           => $post,
                'model'          => $this->_modelPublications
            ]);
