...
abstract class System_Rsa_Keygen {
	 ...
	 */
	private static $_primes = array (4507, 4513, 4517, 4519, 4523, 4547, 4549, 4561, 4567, 4583, 4591, 4597,
	...
	private static function _getMaxPrimes() {
		return count(self::$_primes) - 1;
	}
	...
	*/
	private static function _generateKeys($show_debug=0){

        while (empty($e) || empty($d)) {
                $p = self::$_primes[mt_rand(0, self::_getMaxPrimes())];
                while (empty($q) || ($p==$q)) $q = self::$_primes[mt_rand(0, self::_getMaxPrimes())];
                $n = $p*$q;
                $pi = ($p - 1) * ($q - 1);
                $e = self::_tofindE($pi, $p, $q);
                $d = self::_extend($e,$pi);
                $keys = array ($n, $e, $d);
        }
		//$keys = array(87670783, 9719, 85036607);
        return $keys;
	}
	 ...
	 */
	private static function _extend ($Ee,$Epi) {
        $u1 = 1;
        $u2 = 0;
        $u3 = $Epi;
        $v1 = 0;
	...
	private static function _gcd($e,$pi) {
        $y = $e;
        $x = $pi;
        while ($y != 0) {
                $w =  intval(fmod($x,$y));
	 ...
	 */
	private static function _tofindE($pi) {

        $great = 0;
        $cc = mt_rand (0,self::_getMaxPrimes());
        $startcc = $cc;
        while ($cc >= 0) {
                $se = self::$_primes[$cc];
                $great = self::_gcd($se,$pi);
                $cc--;
                if ($great == 1) break;
        }
        if ($great == 0) {
                $cc = $startcc + 1;
                while ($cc <= self::_getMaxPrimes()) {
                        $se = self::$_primes[$cc];
                        $great = self::_gcd($se,$pi);
                        $cc++;
                        if ($great == 1) break;
	...
	public static function Generate($debug=false) {
        do {
                list ($n, $e, $d) = self::_generateKeys(0);
                if ($test = new System_Rsa_Rsalib($n,$e,$d)) $result = true;
                else $result = false;
        } while (!$result);
		unset($test);
        if ($debug==true) {
		...
		return array('module' => $n, 'public' => $e, 'private' => $d);
	}
